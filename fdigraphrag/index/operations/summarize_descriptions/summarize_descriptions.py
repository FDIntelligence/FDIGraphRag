...
from fdigraphrag.logger.progress import ProgressTicker, progress_ticker
...
async def summarize_descriptions(
    ...
    To turn this feature ON please set the environment variable `fdigraphrag_SUMMARIZE_DESCRIPTIONS_ENABLED=True`.
    ...
    strategy:
        ...
        llm: # The configuration for the LLM
            ...
            api_key: !ENV ${fdigraphrag_OPENAI_API_KEY} # The api key to use for openai
            model: !ENV ${fdigraphrag_OPENAI_MODEL:gpt-4-turbo-preview} # The model to use for openai
            max_tokens: !ENV ${fdigraphrag_MAX_TOKENS:6000} # The max tokens to use for openai
            organization: !ENV ${fdigraphrag_OPENAI_ORGANIZATION} # The organization to use for openai

            # if using azure flavor
            api_base: !ENV ${fdigraphrag_OPENAI_API_BASE} # The api base to use for azure
            api_version: !ENV ${fdigraphrag_OPENAI_API_VERSION} # The api version to use for azure
            proxy: !ENV ${fdigraphrag_OPENAI_PROXY} # The proxy to use for azure
    ```
    """
...
def load_strategy(strategy_type: SummarizeStrategyType) -> SummarizationStrategy:
    ...
    match strategy_type:
        case SummarizeStrategyType.graph_intelligence:
            from fdigraphrag.index.operations.summarize_descriptions.graph_intelligence_strategy import (

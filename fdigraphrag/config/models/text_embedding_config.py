...
from fdigraphrag.config.enums import TextEmbeddingTarget
...
class TextEmbeddingConfig(BaseModel):
    ...
    batch_size: int = Field(
        ...
        default=fdigraphrag_config_defaults.embed_text.batch_size,
    )
    batch_max_tokens: int = Field(
        description="The batch max tokens to use.",
        default=fdigraphrag_config_defaults.embed_text.batch_max_tokens,
    )
    target: TextEmbeddingTarget = Field(
        description="The target to use. 'all', 'required', 'selected', or 'none'.",
        default=fdigraphrag_config_defaults.embed_text.target,
    )
    names: list[str] = Field(
        description="The specific embeddings to perform.",
        default=fdigraphrag_config_defaults.embed_text.names,
    )
    strategy: dict | None = Field(
        description="The override strategy to use.",
        default=fdigraphrag_config_defaults.embed_text.strategy,
    )
    model_id: str = Field(
        description="The model ID to use for text embeddings.",
        default=fdigraphrag_config_defaults.embed_text.model_id,
    )
    vector_store_id: str = Field(
        description="The vector store ID to use for text embeddings.",
        default=fdigraphrag_config_defaults.embed_text.vector_store_id,
    )

    def resolved_strategy(self, model_config: LanguageModelConfig) -> dict:
        """Get the resolved text embedding strategy."""
        from fdigraphrag.index.operations.embed_text import (

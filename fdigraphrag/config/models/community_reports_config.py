...
class CommunityReportsConfig(BaseModel):
    ...
    graph_prompt: str | None = Field(
        ...
        default=fdigraphrag_config_defaults.community_reports.graph_prompt,
    )
    text_prompt: str | None = Field(
        description="The community report extraction prompt to use for text-based summarization.",
        default=fdigraphrag_config_defaults.community_reports.text_prompt,
    )
    max_length: int = Field(
        description="The community report maximum length in tokens.",
        default=fdigraphrag_config_defaults.community_reports.max_length,
    )
    max_input_length: int = Field(
        description="The maximum input length in tokens to use when generating reports.",
        default=fdigraphrag_config_defaults.community_reports.max_input_length,
    )
    strategy: dict | None = Field(
        description="The override strategy to use.",
        default=fdigraphrag_config_defaults.community_reports.strategy,
    )
    model_id: str = Field(
        description="The model ID to use for community reports.",
        default=fdigraphrag_config_defaults.community_reports.model_id,
    )
    ...
    def resolved_strategy(
        ...
        from fdigraphrag.index.operations.summarize_communities.typing import (

...
import fdigraphrag.config.defaults as defs
from fdigraphrag.config.defaults import fdigraphrag_config_defaults
from fdigraphrag.config.errors import LanguageModelConfigMissingError
from fdigraphrag.config.models.basic_search_config import BasicSearchConfig
from fdigraphrag.config.models.cache_config import CacheConfig
from fdigraphrag.config.models.chunking_config import ChunkingConfig
from fdigraphrag.config.models.cluster_graph_config import ClusterGraphConfig
from fdigraphrag.config.models.community_reports_config import CommunityReportsConfig
from fdigraphrag.config.models.drift_search_config import DRIFTSearchConfig
from fdigraphrag.config.models.embed_graph_config import EmbedGraphConfig
from fdigraphrag.config.models.extract_claims_config import ClaimExtractionConfig
from fdigraphrag.config.models.extract_graph_config import ExtractGraphConfig
from fdigraphrag.config.models.extract_graph_nlp_config import ExtractGraphNLPConfig
from fdigraphrag.config.models.global_search_config import GlobalSearchConfig
from fdigraphrag.config.models.input_config import InputConfig
from fdigraphrag.config.models.language_model_config import LanguageModelConfig
from fdigraphrag.config.models.local_search_config import LocalSearchConfig
from fdigraphrag.config.models.output_config import OutputConfig
from fdigraphrag.config.models.prune_graph_config import PruneGraphConfig
from fdigraphrag.config.models.reporting_config import ReportingConfig
from fdigraphrag.config.models.snapshots_config import SnapshotsConfig
from fdigraphrag.config.models.summarize_descriptions_config import (
    SummarizeDescriptionsConfig,
)
from fdigraphrag.config.models.text_embedding_config import TextEmbeddingConfig
from fdigraphrag.config.models.umap_config import UmapConfig
from fdigraphrag.config.models.vector_store_config import VectorStoreConfig
from fdigraphrag.vector_stores.factory import VectorStoreType


class fdigraphragConfig(BaseModel):
    ...
    root_dir: str = Field(
        ...
        default=fdigraphrag_config_defaults.root_dir,
    )
    ...
    models: dict[str, LanguageModelConfig] = Field(
        ...
        default=fdigraphrag_config_defaults.models,
    )
    ...
    def _validate_reporting_base_dir(self) -> None:
        ...
        if self.reporting.type == defs.ReportingType.file:
            if self.reporting.base_dir.strip() == "":
                msg = "Reporting base directory is required for file reporting. Please rerun `fdigraphrag init` and set the reporting configuration."
    ...
    def _validate_output_base_dir(self) -> None:
        ...
        if self.output.type == defs.OutputType.file:
            if self.output.base_dir.strip() == "":
                msg = "output base directory is required for file output. Please rerun `fdigraphrag init` and set the output configuration."
                raise ValueError(msg)
    ...
    outputs: dict[str, OutputConfig] | None = Field(
        ...
        default=fdigraphrag_config_defaults.outputs,
    )
    ...
    update_index_output: OutputConfig = Field(
        ...
        default=OutputConfig(
            type=fdigraphrag_config_defaults.update_index_output.type,
            base_dir=fdigraphrag_config_defaults.update_index_output.base_dir,
        ),
    )
    ...
    def _validate_update_index_output_base_dir(self) -> None:
        ...
        if self.update_index_output.type == defs.OutputType.file:
            if self.update_index_output.base_dir.strip() == "":
                msg = "update_index_output base directory is required for file output. Please rerun `fdigraphrag init` and set the update_index_output configuration."
                raise ValueError(msg)
    ...
    extract_claims: ClaimExtractionConfig = Field(
        ...
        default=ClaimExtractionConfig(
            enabled=fdigraphrag_config_defaults.extract_claims.enabled,
        ),
    )
    ...
    vector_store: dict[str, VectorStoreConfig] = Field(
        ...
        default_factory=lambda: {
            ...
            for k, v in fdigraphrag_config_defaults.vector_store.items()
        },
    )
    ...
    workflows: list[str] | None = Field(
        ...
        default=fdigraphrag_config_defaults.workflows,
    )
        ...
        for store in self.vector_store.values():
            if store.type == VectorStoreType.LanceDB:
                if not store.db_uri or store.db_uri.strip == "":
                    msg = "Vector store URI is required for LanceDB. Please rerun `fdigraphrag init` and set the vector store configuration."
                    raise ValueError(msg)
    ...
    def get_language_model_config(self, model_id: str) -> LanguageModelConfig:
        ...
        if model_id not in self.models:
            err_msg = f"Model ID {model_id} not found in configuration. Please rerun `fdigraphrag init` and set the model configuration."
    ...
    def get_vector_store_config(self, vector_store_id: str) -> VectorStoreConfig:
        ...
        if vector_store_id not in self.vector_store:
            err_msg = f"Vector Store ID {vector_store_id} not found in configuration. Please rerun `fdigraphrag init` and set the vector store configuration."
            raise ValueError(err_msg)

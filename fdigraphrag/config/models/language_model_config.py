...
from fdigraphrag.config.defaults import language_model_defaults
from fdigraphrag.config.enums import AsyncType, AuthType, ModelType
from fdigraphrag.config.errors import (
    ApiKeyMissingError,
...
from fdigraphrag.language_model.factory import ModelFactory
...
class LanguageModelConfig(BaseModel):
    ...
    def _validate_api_key(self) -> None:
        ...
        if (self.auth_type == AuthType.AzureManagedIdentity) and (
            ...
            msg = "API Key should not be provided when using Azure Managed Identity. Please rerun `fdigraphrag init` and remove the api_key when using Azure Managed Identity."
    ...
    def _validate_auth_type(self) -> None:
        ...
        if self.auth_type == AuthType.AzureManagedIdentity and (
            ...
            msg = f"auth_type of azure_managed_identity is not supported for model type {self.type}. Please rerun `fdigraphrag init` and set the auth_type to api_key."
            raise ConflictingSettingsError(msg)

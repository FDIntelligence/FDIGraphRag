...
from fdigraphrag.config.defaults import vector_store_defaults
...
class VectorStoreConfig(BaseModel):
    ...
    def _validate_db_uri(self) -> None:
        ...
        if self.type != VectorStoreType.LanceDB.value and (
            ...
            msg = "vector_store.db_uri is only used when vector_store.type == lancedb. Please rerun `fdigraphrag init` and select the correct vector store type."
    ...
    def _validate_url(self) -> None:
        ...
        if self.type == VectorStoreType.AzureAISearch and (
            ...
            msg = "vector_store.url is required when vector_store.type == azure_ai_search. Please rerun `fdigraphrag init` and select the correct vector store type."
            raise ValueError(msg)

        if self.type == VectorStoreType.CosmosDB and (
            self.url is None or self.url.strip() == ""
        ):
            msg = "vector_store.url is required when vector_store.type == cosmos_db. Please rerun `fdigraphrag init` and select the correct vector store type."
            raise ValueError(msg)

        if self.type == VectorStoreType.LanceDB and (
            self.url is not None and self.url.strip() != ""
        ):
            msg = "vector_store.url is only used when vector_store.type == azure_ai_search or vector_store.type == cosmos_db. Please rerun `fdigraphrag init` and select the correct vector store type."
            raise ValueError(msg)

...
class GlobalSearchConfig(BaseModel):
    ...
    map_prompt: str | None = Field(
        ...
        default=fdigraphrag_config_defaults.global_search.map_prompt,
    )
    reduce_prompt: str | None = Field(
        description="The global search reducer to use.",
        default=fdigraphrag_config_defaults.global_search.reduce_prompt,
    )
    chat_model_id: str = Field(
        description="The model ID to use for global search.",
        default=fdigraphrag_config_defaults.global_search.chat_model_id,
    )
    knowledge_prompt: str | None = Field(
        description="The global search general prompt to use.",
        default=fdigraphrag_config_defaults.global_search.knowledge_prompt,
    )
    temperature: float = Field(
        description="The temperature to use for token generation.",
        default=fdigraphrag_config_defaults.global_search.temperature,
    )
    top_p: float = Field(
        description="The top-p value to use for token generation.",
        default=fdigraphrag_config_defaults.global_search.top_p,
    )
    n: int = Field(
        description="The number of completions to generate.",
        default=fdigraphrag_config_defaults.global_search.n,
    )
    max_tokens: int = Field(
        description="The maximum context size in tokens.",
        default=fdigraphrag_config_defaults.global_search.max_tokens,
    )
    data_max_tokens: int = Field(
        description="The data llm maximum tokens.",
        default=fdigraphrag_config_defaults.global_search.data_max_tokens,
    )
    map_max_tokens: int = Field(
        description="The map llm maximum tokens.",
        default=fdigraphrag_config_defaults.global_search.map_max_tokens,
    )
    reduce_max_tokens: int = Field(
        description="The reduce llm maximum tokens.",
        default=fdigraphrag_config_defaults.global_search.reduce_max_tokens,
    )
    concurrency: int = Field(
        description="The number of concurrent requests.",
        default=fdigraphrag_config_defaults.global_search.concurrency,
    )
    ...
    dynamic_search_max_level: int = Field(
        ...
        default=fdigraphrag_config_defaults.global_search.dynamic_search_max_level,
    )

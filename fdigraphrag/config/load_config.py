...
from fdigraphrag.config.create_fdigraphrag_config import create_fdigraphrag_config
...
def _get_config_path(root_dir: Path, config_filepath: Path | None) -> Path:
    ...
    """
    print(f"root_dir: {root_dir} and also config_filepath: {config_filepath}")
    if config_filepath:
...
def load_config(
    ...
    return create_fdigraphrag_config(config_data, root_dir=str(root))
